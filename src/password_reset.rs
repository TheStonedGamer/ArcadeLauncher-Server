...
fn reset_form_page(token: &str, error: Option<&str>) -> Response {
    ...
    let html = format!(
        "<!doctype html><html><head><meta charset=\"utf-8\"><title>Reset password</title>\
         ...
         <input id=\"p2\" type=\"password\" minlength=\"10\" required autocomplete=\"new-password\">\
         ...
         b=document.getElementById('p2').value;if(a.length<10){{alert('Password must be at \
         least 10 characters.');return false}}if(a!==b){{alert('Passwords do not match.');\
...
async fn api_auth_reset_submit(
    ...
    if form.password.len() < 10 {
        return reset_form_page(&token, Some("Password must be at least 10 characters."));
    }

...
async fn ensure_social_schema(c: &mut mysql_async::Conn) -> Result<()> {
    ...
    .await?;
    // NOTE: built-in game streaming was removed (client v0.13.22). The tables it
    // used — `stream_hosts`, `stream_client_certs`, `stream_host_apps` — are no
    // longer created or read. Existing deployments keep their rows (dropping them
    // is a manual, deliberate DBA step; nothing here touches user data).
    Ok(())
}
